...
) mh2 ON mh1.museum_id = mh2.min_id
WHERE mh2.min_id IS NULL;

select count(museum_id) from museum_hours;
...
-- 16) Identify the artists whose paintings are displayed in multiple countries
    ...
    with cte_country as 
		...
		cte_city as
			...
			from museum
			group by city)
	select group_concat( distinct country.country) as Country_Name  , group_concat(city.city,', ') as City_Name
